...
def execute(filters=None): 
	...
	for sle in sl_entries:
		...
		a = (frappe.db.get_value("Batch", sle.get('batch_no'), "manufacturing_date"))
		b = (frappe.db.get_value("Batch", sle.get('batch_no'), "expiry_date"))
		sle.update({
		
			"in_qty": max(sle.actual_qty, 0),
			"out_qty": min(sle.actual_qty, 0),
			"manufacturing_date": (frappe.db.get_value("Batch", sle.get('batch_no'), "manufacturing_date")), 
			"expiry_date": (frappe.db.get_value("Batch", sle.get('batch_no'), "expiry_date")),
			"shelf_life" : date_diff(b,a)
		})

		if filters.get("date_of_generating_report"):
			sle.update({
				"lapsed_life":date_diff(filters.get("date_of_generating_report"),a),
				"balanced_life":date_diff(b,filters.get("date_of_generating_report"))
	...
	for d in data:
		if d.get("item_code") and frappe.db.exists("Item", d.get("item_code")):
			...
			if values_dict:
				...
				d["item_weight"] = flt(d.get("qty_after_transaction")) * flt(item_weight)
				d["brand"] = brand
				d["cases"] = (flt(d.get("qty_after_transaction")) / flt(conv_factor)) if conv_factor else 0
...
def get_columns():
	columns = [
		...
		{"label": _("Balanced Value"), "fieldname": "stock_value", "fieldtype": "Currency", "width": 110, "options": "Company:company:default_currency"},
		...
		{"label": _("Shelf Life"), "fieldname": "shelf_life", "fieldtype": "Int", "width": 110},
		{"label": _("Lapsed Life"), "fieldname": "lapsed_life", "fieldtype": "Int", "width": 110},
		{"label": _("Balanced Life"), "fieldname": "balanced_life", "fieldtype": "Int", "width": 110}]
	...
	sl_entries = frappe.db.sql("""
		...
		WHERE
			...
			sle.company = %(company)s
				AND sle.is_cancelled = 0 AND batch_no != "" AND sle.posting_date BETWEEN %(from_date)s AND %(to_date)s
				{sle_conditions}
				{item_conditions_sql}
		ORDER BY
...
def get_items_for_range():
	query = frappe.db.sql("select item_name from tabItem",as_dict=1)
	return query
