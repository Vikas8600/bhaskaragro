...
from bhaskaragro.bhaskaragro.report.trial_balance_without_cancellation_entries.trial_balance_without_cancellation_entries import validate_filters
	...
	gle = frappe.db.sql("""
		...
		where company=%(company)s
			...
			and (posting_date < %(from_date)s and is_cancelled = 0
			 or ifnull(is_opening, 'No') = 'Yes') and is_cancelled = 0
			{account_filter}
...
def get_balances_within_period(filters):
	...
	gle = frappe.db.sql("""
		...
		where company=%(company)s
			...
			and ifnull(is_opening, 'No') = 'No' and is_cancelled = 0
			{account_filter}
		group by party""".format(account_filter=account_filter), {
			"company": filters.company,
			"from_date": filters.from_date,
