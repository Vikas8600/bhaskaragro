...
def execute(filters=None):
	...
	group_wise_columns = frappe._dict({
		"invoice": ["invoice_or_item", "customer", "customer_group", "posting_date","item_code", "item_name","item_group", "brand", "description",
			"warehouse", "qty", "alternate_qty", "alternate_uom", "weight", "weight_uom", "base_rate", "buying_rate", "base_amount",
			"buying_amount", "gross_profit", "gross_profit_percent", "project"],

		"item_code": ["item_code", "item_name", "brand", "description", "qty",
			"alternate_qty", "alternate_uom", "weight", "weight_uom", "base_rate",
...
def get_columns(group_wise_columns, filters):
	...
	column_map = frappe._dict({
		...
		"item_code": _("Item Code") + ":Link/Item:200",
		"item_name": _("Item Name") + ":Data:200",
		"item_group": _("Item Group") + ":Link/Item Group:100",
		"brand": _("Brand") + ":Link/Brand:100",
		"description": _("Description") +":Data:200",
		"warehouse": _("Warehouse") + ":Link/Warehouse:100",
		"qty": _("Qty") + ":Float:80",

		"alternate_qty": _("Alternate Qty") + ":Float:100",
		"alternate_uom": _("Alternate UOM") + ":Link/UOM:100",
		"weight": _("Weight") + ":Float:100",
		"weight_uom": _("Weight UOM") + ":Link/UOM:100", 
...
class GrossProfitGenerator(object):
	...
	def get_returned_invoice_items(self):
		returned_invoices = frappe.db.sql("""
			select
				...
				case 
                When si_item.stock_qty
                Then (Select weight_uom from `tabItem` where item_code = si_item.item_code )
                End as weight_uom, si.return_against
			from
	...
	def load_invoice_items(self):
		...
		self.si_list = frappe.db.sql("""
			select
				...
				(Select (`tabSales Invoice Item`.stock_qty * i.weight_per_unit)
				from `tabItem` i where i.item_code = `tabSales Invoice Item`.name ) as weight,	
	...
	def load_stock_ledger_entries(self):
		res = frappe.db.sql("""select item_code, voucher_type, voucher_no,
				...
				(Select `tabStock Ledger Entry`.actual_qty * i.weight_per_unit
				from `tabItem` i where i.item_code = `tabStock Ledger Entry`.item_code)
