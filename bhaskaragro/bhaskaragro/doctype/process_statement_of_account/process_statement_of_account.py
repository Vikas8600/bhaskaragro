...
class ProcessStatementofAccount(Document):
	def validate(self):
		if not self.subject:
			self.subject = 'Statement Of Account for {{ customer.name }}'
		if not self.body:
			self.body = 'Hello {{ customer.name }},<br>PFA your Statement Of Account from {{ doc.from_date }} to {{ doc.to_date }}.'
...
def get_report_pdf(doc, consolidated=True):
	...
	template_path = ("bhaskaragro/bhaskaragro/doctype/process_statement_of_account/process_statement_of_account.html")
	print("****************************************************************",doc.customers)
...
def download_statements(document_name):
	doc = frappe.get_doc('Process Statement of Account', document_name)
	...
	if report:
		for customer, report_pdf in report.items():
			...
			frappe.enqueue(
				...
				message=message,
				now=True,
				reference_doctype='Process Statement Of Account',
...
@frappe.whitelist()
def send_auto_email():
	selected = frappe.get_list('Process Statement Of Account', filters={'to_date': format_date(today()), 'enable_auto_email': 1})
